    ...
    <article key={node.slug} className="row post-list-item">
      <div className="col-sm-6 post-list-image">
        <Link style={{ boxShadow: `none` }} to={`/post/${node.slug}`}>
          ...
          <img src={`https://api.flotiq.com/image/460x300/${node.featuredMedia[0].id}.${node.featuredMedia[0].extension}`} alt="test" style={{maxWidth: '100%', height: 'auto'}}/>
          }
        </Link>
      </div>
